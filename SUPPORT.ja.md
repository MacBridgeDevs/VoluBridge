...
VoluBridge は Apple Silicon Mac の macOS 11 以降に対応しています。
...
`System Output` が `VoluBridge` になっていること、`VoluBridge Output` が目的の実デバイスを指していること、VoluBridge ドライバがインストール済みで最新であることを確認してください。デバイス変更後に問題が出た場合は、`Reset Audio Connection` を実行してください。
...
可能な場合は VoluBridge の `Remove App Completely` を使用してください。アプリ、ドライバ、関連設定を削除します。
